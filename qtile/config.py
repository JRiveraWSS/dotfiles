...
def top_bar():
    return bar.Bar(
        [
            ...
            widget.PulseVolume(
                channel="Master", fmt=" {}", foreground=colors["fg"], padding=6
            ),
            widget.Battery(
                format="{percent:2.0%}",
                foreground=colors["fg"],
            ...
            widget.Systray(icon_size=18, padding=6),
            widget.Clock(
                format="%Y-%m-%d  %H:%M", foreground=colors["magenta"], padding=8
            ),
            widget.TextBox(
                text="",
